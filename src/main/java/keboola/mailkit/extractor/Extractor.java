...
public class Extractor {
	...
	public static void main(String[] args) {
		...
		MailkitJsonAPIClient jsonClient = new MailkitJsonAPIClient(config.getParams().getClientId(),
				config.getParams().getClientMd5(), dataPath + File.separator + "tmp");
		if (LOG) {
		...
		try {
			...
			for (String cId : campaignIds) {
				...
				jsonRq = new ReportCampaign(config.getParams().getDateFrom(),
						config.getParams().getDateTo(), cId);
				jsResp = (MailkitJsonResponse) jsonClient.executeRequest(jsonRq, LOG);
			...
			for (String cId : campaignIds) {
				...
				if (datasetsToGet.contains(KBCParameters.REQUEST_TYPE.RAW_MESSAGES.name())) {
					...
					while (hasNextData) {
						...
						if (!checkResponseStatus(jsResp, jsonRq)) {
							System.err.println(
									"Unable to retrieve all records. Will retry on next run starting with ID: "
											+ lastId);
							break;
			...
			for (String sId : sendIds) {
				if (config.getParams().getDatasets()
						.contains(KBCParameters.REQUEST_TYPE.MSG_BOUNCES.name())) {
					Thread.sleep(REQUEST_WAIT_INTERVAL);
				...
				if (config.getParams().getDatasets()
						.contains(KBCParameters.REQUEST_TYPE.MSG_FEEDBACK.name())) {
					Thread.sleep(REQUEST_WAIT_INTERVAL);
				...
				if (config.getParams().getDatasets()
						.contains(KBCParameters.REQUEST_TYPE.MSG_RECIPIENTS.name())) {
					Thread.sleep(REQUEST_WAIT_INTERVAL);
					...
					List<MsgRecipientsResp> reps = jsResp
							.getResponseObject(MsgRecipientsResp.class);
					msgRecipientsWriter
							.writeAllResults(MsgRecipientWrapper.Builder.build(reps, sId));
				}

				if (config.getParams().getDatasets()
						.contains(KBCParameters.REQUEST_TYPE.REPORT_MSG.name())) {
					Thread.sleep(REQUEST_WAIT_INTERVAL);
				...
				if (config.getParams().getDatasets()
						.contains(KBCParameters.REQUEST_TYPE.MSG_LINKS.name())) {
					Thread.sleep(REQUEST_WAIT_INTERVAL);
					...
					// link visitors
					if (config.getParams().getDatasets()
							.contains(KBCParameters.REQUEST_TYPE.LINKS_VISITORS.name())) {
						for (String urlId : linkIds) {
							...
							List<MsgLinksVisitorResp> repsVis = jsResp
									.getResponseObject(MsgLinksVisitorResp.class);
							msgLinkVisitorsWriter.writeAllResults(
									MsgLinkVisitorWrapper.Builder.build(repsVis, sId, urlId));
						}
					}
				}
	...
	private static void initWriters(KBCConfig config, String outTablesPath) throws Exception {
		...
		if (config.getParams().getDatasets().contains(KBCParameters.REQUEST_TYPE.REPORT.name())) {
			...
			reportWriter.initWriter(outTablesPath,
					keboola.mailkit.extractor.mailkitapi.response.ReportResp.class);
		}
